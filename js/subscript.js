...
// Masonry 제거: .js-masonry-modal-gallery 클래스 참조 제거
const modalGalleryGrid = imageGalleryModal ? imageGalleryModal.querySelector('.modal-gallery-grid') : null;

// (수정) 이미지 지연 로딩 (Lazy Loading) 기능에 사용되는 imageObserver를 전역으로 하나만 선언
const imageObserver = new IntersectionObserver((entries, observer) => {
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            ...
            if (dataSrc) {
                img.src = dataSrc; // data-src의 실제 이미지 로드
                img.onload = () => {
                    img.classList.add('loaded'); // 로드 완료 클래스 추가
                };
                img.onerror = () => {
                    console.error('이미지 로드 실패:', img.dataset.src);
                    img.src = 'https://placehold.co/150x150/AAAAAA/FFFFFF?text=Error'; // 오류 시 대체 이미지
                };
            }
            observer.unobserve(img); // 한 번 로드되면 더 이상 관찰하지 않음
        }
    });
}, { rootMargin: '0px 0px 100px 0px' }); // 뷰포트에서 100px 미리 로드 시작

// 이미지 갤러리 모달 열기 함수
const openImageGalleryModal = (title, imagesArray) => {
    if (!galleryTitle || !modalGalleryGrid || !imageGalleryModal) {
        console.warn('갤러리 모달 관련 DOM 요소가 없어 이미지 갤러리를 열 수 없습니다.');
        return; // 요소 존재 여부 확인
    }
    ...
    imagesArray.forEach(imgSrc => {
        ...
        img.src = "data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs="; // tiny placeholder to ensure src attribute exists
    ...
    // 모달의 CSS 전환이 완료될 시간을 벌어줍니다.
    setTimeout(() => {
        // --- Masonry.js 관련 코드 제거 ---
        // 이미지가 로드될 때마다 지연 로딩 트리거
        newImageElements.forEach(img => {
            // 지연 로딩을 위해 Intersection Observer 트리거
            imageObserver.observe(img);
        });
    }, 400); // 모달 전환 애니메이션 시간을 고려하여 충분한 지연
};
    ...
    if (categoryModalTitle && categoryProductGrid && productCategoryModal) { // 요소 존재 여부 확인
        ...
        for (const modelId in category.models) {
            ...
            productCard.innerHTML = `
                <img src="data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=" data-src="${model.thumbnail}" alt="${model.displayName}" loading="lazy">
        ...
        console.warn('제품 카테고리 모달 관련 DOM 요소가 없어 모달을 열 수 없습니다.');
    }
};
...
// 이 imageObserver 선언을 유지하고, 상단의 중복 선언을 제거합니다.
